...
const Search = () => {
  ...
  const searchResults = useQuery(api.documents.searchDocuments, { query: searchText }) || [];
    ...
    <div className="mx-auto my-8 bg-white shadow-lg rounded-lg p-6 w-full max-w-md">
      ...
      <ul className="list-none space-y-2">
        {searchResults.map((searchResult) => (
          ...
          >
            <span className="font-medium text-gray-800">{searchResult.title}</span>
            <span className="block text-sm text-gray-500 pt-1">
              Created at: {new Date(searchResult._creationTime).toLocaleString()}
            </span>
          </li>
        ))}
      </ul>
